...
# Starting to run the rsync command. All output is captured into a file at the end.
	...
	# Copy the files 
	echo /usr/bin/rsync -ruvap --progress djveer@198.1.62.19:/srv/northwest.ca/ /srv/websites/northwest.ca-$(date +%d-%m-%Y)
	/usr/bin/rsync -ruvap --progress djveer@198.1.62.19:/srv/northwest.ca/ /srv/websites/northwest.ca-$(date +%d-%m-%Y) >> $RSYNC_OUTPUT 2>&1
	rc=$?
